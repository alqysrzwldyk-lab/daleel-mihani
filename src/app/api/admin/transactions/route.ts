...
const TYPES = ["payment", "commission", "withdrawal", "deposit", "subscription", "boost"];
...
export async function GET(req: NextRequest) {
  ...
  if (user) {
    ...
    if (userFilterIds.length) {
      filter.$or = [{ fromUserId: { $in: userFilterIds } }, { toUserId: { $in: userFilterIds } }];
    } else {
      return NextResponse.json({ transactions: [], total: 0, totalIncome: 0, page, totalPages: 1 });
    }
  }
  ...
  try {
    ...
    return NextResponse.json({
      transactions: txns.map((t) => ({
        ...
        fromName: t.fromUserId ? userMap.get(String(t.fromUserId))?.name || "" : "",
        toName: t.toUserId ? userMap.get(String(t.toUserId))?.name || "" : "",
        type: t.type,
        amount: t.amount,
        fee: t.fee,
        currency: t.currency,
