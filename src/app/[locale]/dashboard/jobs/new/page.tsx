...
export default function NewJobPage() {
  ...
  if (loading) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-12 flex items-center justify-center gap-3 text-[var(--muted)]">
        ...
        <span className="text-sm font-bold">{T("جاري التحقق من الحساب...")}</span>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        <h1 className="text-2xl md:text-3xl font-black text-[var(--foreground)]">{T("إضافة إعلان وظيفي")}</h1>
        <p className="text-sm text-[var(--muted)] mt-1.5">{T("املأ تفاصيل الوظيفة بدقة ليصل إشعار للمهنيين المناسبين ويصلتك أفضل المتقدمين.")}</p>
      </div>
