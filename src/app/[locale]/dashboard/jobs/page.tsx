...
export default function CompanyJobsPage() {
  ...
  async function handleDelete(job: JobItem) {
    if (!confirm(T('هل أنت متأكد من حذف إعلان "{title}" وكل طلبات التقديم المرتبطة به؟', { title: job.jobTitle }))) return;
    const res = await fetch(`/api/jobs/${job._id}`, { method: "DELETE" });
    const data = await res.json();
    if (res.ok) {
      setJobs((prev) => prev.filter((j) => j._id !== job._id));
    } else {
      alert(data.error ? T(data.error) : T("حدث خطأ أثناء الحذف"));
    }
  }
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-[var(--muted)] mt-1.5">{T("إدارة إعلانات التوظيف الخاصة بشركتك ومتابعة المتقدمين عليها.")}</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl card-shadow p-5 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs font-bold text-[var(--muted)]">{T("إجمالي الإعلانات")}</p>
          </div>
        </div>
        <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl card-shadow p-5 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs font-bold text-[var(--muted)]">{T("إعلانات مفتوحة")}</p>
          </div>
        </div>
        <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl card-shadow p-5 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs font-bold text-[var(--muted)]">{T("إجمالي المتقدمين")}</p>
          </div>
        </div>
      </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center py-24 gap-3 text-[var(--muted)]">
          ...
          <span className="text-sm font-bold">{T("جاري تحميل إعلاناتك...")}</span>
        </div>
      ) : jobs.length === 0 ? (
        <div className="text-center py-20 border-2 border-dashed border-[var(--border)] rounded-2xl bg-[var(--card)]">
          <div className="text-5xl mb-4">💼</div>
          <h3 className="font-black text-lg text-[var(--foreground)]">{T("لا توجد إعلانات وظيفية بعد")}</h3>
          <p className="text-sm text-[var(--muted)] mt-2 mb-6">{T("ابدأ بنشر أول إعلان وظيفي لتستقبل طلبات المهنيين المناسبين.")}</p>
          <Link href="/dashboard/jobs/new" className="inline-flex items-center gap-2 bg-[var(--primary)] text-white font-bold px-6 py-3 rounded-xl transition hover:bg-[var(--primary-dark)]">
            <Plus className="w-4 h-4" /> {T("إضافة إعلان وظيفي")}
          </Link>
        </div>
      ) : (
          ...
          {jobs.map((job) => (
            <div key={job._id} className="bg-[var(--card)] border border-[var(--border)] rounded-2xl card-shadow p-5 flex flex-col md:flex-row md:items-center gap-4 hover:border-[var(--primary)] transition">
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-1.5">
                  <span className={`text-[11px] font-bold px-2.5 py-0.5 rounded-full ${job.status === "open" ? "bg-[var(--success)]/15 text-[var(--success)] border border-[var(--success)]/25" : "bg-[var(--surface)] text-[var(--muted)] border border-[var(--border)]"}`}>
                    {job.status === "open" ? T("🟢 مفتوح") : T("🔴 مغلق")}
                  </span>
                ...
                <div className="flex flex-wrap items-center gap-4 mt-1.5 text-xs text-[var(--muted)] font-bold">
                  <span className="inline-flex items-center gap-1"><Users className="w-3.5 h-3.5" /> {T("{count} متقدم", { count: job.applicationsCount })}</span>
                  <span className="inline-flex items-center gap-1"><Eye className="w-3.5 h-3.5" /> {T("{count} مشاهدة", { count: job.views })}</span>
                  <span>{job.city}</span>
              ...
              <div className="flex items-center gap-2 md:shrink-0 flex-wrap">
                ...
                >
                  <Users className="w-3.5 h-3.5" /> {T("المتقدمين")}
                </Link>
                <button
                ...
                >
                  <Trash2 className="w-3.5 h-3.5" /> {T("حذف")}
                </button>
              </div>
            </div>
          ))}
