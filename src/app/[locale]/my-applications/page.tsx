  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-sm text-[var(--muted)] mt-1.5">متابعة حالة طلبات التقديم التي أرسلتها على الوظائف.</p>
      </div>

      <div className="flex flex-wrap items-center gap-3 mb-6">
        <span className="bg-[var(--card)] border border-[var(--border)] rounded-xl px-4 py-2 text-xs font-bold text-[var(--foreground)]">إجمالي الطلبات: {apps.length}</span>
        <span className="bg-[var(--success)]/15 border border-[var(--success)]/25 rounded-xl px-4 py-2 text-xs font-bold text-[var(--success)]">مقبولة: {accepted}</span>
        <span className="bg-[var(--warning)]/15 border border-[var(--warning)]/25 rounded-xl px-4 py-2 text-xs font-bold text-[var(--warning)]">قيد المراجعة: {pending}</span>
      </div>

      {loading ? (
        <div className="flex flex-col items-center py-24 gap-3 text-[var(--muted)]">
          <Loader2 className="w-8 h-8 animate-spin text-[var(--primary)]" />
          <span className="text-sm font-bold">جاري تحميل طلباتك...</span>
        </div>
      ) : apps.length === 0 ? (
        <div className="text-center py-20 border-2 border-dashed border-[var(--border)] rounded-2xl bg-[var(--card)]">
          <div className="text-5xl mb-4">📋</div>
          <h3 className="font-black text-lg text-[var(--foreground)]">لم تتقدم على أي وظيفة بعد</h3>
          <p className="text-sm text-[var(--muted)] mt-2 mb-6">تصفح فرص العمل المتاحة وقدّم على ما يناسب مهاراتك.</p>
              ...
              <div key={app._id} className="bg-[var(--card)] border border-[var(--border)] rounded-2xl card-shadow p-5 flex flex-col sm:flex-row sm:items-center gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    ...
                    <span className={`text-[11px] font-bold px-2.5 py-0.5 rounded-full ${meta.classes}`}>{meta.icon} {meta.label}</span>
                  </div>
                  ...
                  <span className="inline-flex items-center gap-1 text-xs text-[var(--muted)] font-bold mt-1.5">
                    <CalendarDays className="w-3.5 h-3.5" /> قدّمت بتاريخ: {formatDate(app.createdAt)}
                  </span>
                </div>
                <div className="shrink-0 flex items-center gap-2">
                  ...
                  <Link href={`/jobs/${app.jobId}`} className="inline-flex items-center gap-1.5 text-xs font-bold text-[var(--primary)] border border-[var(--primary)]/20 bg-[var(--primary)]/5 hover:bg-[var(--primary)] hover:text-white px-4 py-2.5 rounded-xl transition">
                    <ExternalLink className="w-3.5 h-3.5" /> عرض الوظيفة
                  </Link>
                </div>
              </div>
            );
