...
export async function generateMetadata(): Promise<Metadata> {
  const locale =
    (await cookies()).get("NEXT_LOCALE")?.value === "en" ? "en" : "ar";

  const siteName = tr("الدليل المهني", locale);
  const tagline = `${tr("كل المهنيين", locale)} ${tr(
    "في مكان واحد",
    locale
  )}`;
  ...
  const description = tr(
    "منصة تجمع المهنيين، الحرفيين، الطلاب، والخبراء مع أصحاب الأعمال لتسهيل الوصول إلى الكفاءات وبناء فرص مهنية حقيقية.",
    locale
  );

  const shortDescription = tr(
    "منصة تجمع المهنيين، الحرفيين، الطلاب، والخبراء مع أصحاب الأعمال لتسهيل الوصول إلى الكفاءات.",
    locale
  );
