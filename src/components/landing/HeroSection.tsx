      ...
      <div className="page-container relative z-10">
        <div className={`max-w-3xl mx-auto text-center transition-all duration-700 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full px-4 py-1.5 mb-6">
            ...
            <span className="text-white/80 text-sm">{T("منصة المهنيين الأولى في العالم العربي")}</span>
          </div>
          ...
          <p className="text-lg md:text-xl text-white/70 max-w-2xl mx-auto mb-8 leading-relaxed">
            {T("منصة تجمع المهنيين، الحرفيين، الطلاب، والخبراء مع أصحاب الأعمال والعملاء لتسهيل الوصول إلى الكفاءات وبناء فرص مهنية حقيقية.")}
          </p>
          ...
          >
            <div className="flex-1 flex items-center px-4">
              ...
              <input
                ...
                placeholder={T("ابحث عن طبيب، مهندس، معلم، شركة، أو أي مهنة...")}
          ...
          <div className="flex flex-wrap justify-center gap-2 mb-8">
            <span className="text-white/50 text-sm ml-1">{T("الأكثر بحثاً:")}</span>
            {POPULAR.map((item) => (
              <Link
                key={item}
