...
export default function MessageCompanyModal({
  ...
  const send = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || T("فشل إرسال الرسالة"));
        return;
      }
      onClose();
      ...
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-black text-[var(--foreground)]">
            {T("مراسلة {name}", { name: companyName })}
          </h3>
        ...
        <p className="text-sm text-[var(--muted)] mb-4">
          {T("اكتب رسالتك وسيصلها مباشرة إلى فريق الشركة عبر رسائل الدليل.")}
        </p>

        <textarea
          value={content}
          onChange={(e) => setContent(e.target.value)}
          placeholder={T("مثال: أرغب بالاستفسار عن فرص العمل لدى شركتكم...")}
